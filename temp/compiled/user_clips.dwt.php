...
<?php echo $this->smarty_insert_scripts(array('files'=>'transport_org.js,common.js,user.js,car_org.js')); ?>
</head>
<body>
      ...
      <div class="userCenterBox boxCenterList clearfix" style="_height:1%;">
         ...
         <?php if ($this->_var['action'] == 'default'): ?>
          ...
          <?php echo $this->_var['lang']['not_validated']; ?> <a href="javascript:sendHashMail()" style="color:#006bd0;"><?php echo $this->_var['lang']['resend_hash_mail']; ?></a><br />
           ...
           <img src="themes/car/images/note.gif" alt="note" />&nbsp;<?php echo $this->_var['user_notice']; ?>
           </div>
           <br /><br />
...
?>
         ...
         <?php if ($this->_var['action'] == 'icar'): ?>
         <h5><span>我的车型</span></h5>
      ...
      <tr align="center">
        ...
        <th width="15%" bgcolor="#ffffff">默认车型</th>
        <th width="15%" bgcolor="#ffffff">操作</th>
      </tr>
       <?php $_from = $this->_var['icar']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'icar');if (count($_from)):
    foreach ($_from AS $this->_var['icar']):
?>
      <tr>
        <td bgcolor="#ffffff" align="center"><?php echo $this->_var['icar']['cbrand_name']; ?></td>
        <td bgcolor="#ffffff" align="center"><?php echo $this->_var['icar']['csys_name']; ?></td>
        <td bgcolor="#ffffff" align="center"><?php echo $this->_var['icar']['ctype_name']; ?></td>
        <td bgcolor="#ffffff" align="center"><?php if ($this->_var['icar']['defaultcar'] == '0'): ?>否<?php else: ?>是<?php endif; ?></td>
        <td bgcolor="#ffffff" align="center"><a href="user.php?act=edit_icar&id=<?php echo $this->_var['icar']['id']; ?>">修改</a>&nbsp;|&nbsp;<a onclick="javascript:delicar(<?php echo $this->_var['icar']['id']; ?>)" href="javascript:void(0)">删除</a></td>
      </tr>
	<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
    <form method="post" action="user.php?act=add_icar" >
    <tr>
...
?>
            <option value="<?php echo $this->_var['cb']['id']; ?>"><?php echo $this->_var['cb']['cbrand_name']; ?></option>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
            </select>
        </td>
        <td bgcolor="#ffffff" align="center">
        ...
        <td bgcolor="#ffffff" align="center">
        <input type="checkbox" name="defaultcar" value="1" >是
        </td>
        <td bgcolor="#ffffff" align="center">
    ...
    <?php endif; ?>
    <?php if ($this->_var['action'] == 'edit_icar'): ?>
         <h5><span>编辑<?php echo $this->_var['icar']['ctype_name']; ?></span></h5>     
     <table width="100%" border="0" cellpadding="5" cellspacing="1" bgcolor="#dddddd">
     <tr align="center">
        <th width="20%" bgcolor="#ffffff">品牌</th>
        <th width="20%" bgcolor="#ffffff">车系</th>
    ...
    <form method="post" action="user.php?act=edit_done" >
    <tr>
        <td bgcolor="#ffffff" align="center">
        <select onchange="callbrand(this.value)" name="cb_name">
            <option value="">请选择品牌</option>
    ...
    foreach ($_from AS $this->_var['cb']):
?>
            <option <?php if ($this->_var['icar']['cbrand'] == $this->_var['cb']['id']): ?>selected="selected"<?php endif; ?> value="<?php echo $this->_var['cb']['id']; ?>"><?php echo $this->_var['cb']['cbrand_name']; ?></option>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
            </select>
        </td>
        <td bgcolor="#ffffff" align="center">
...
?>
            <option <?php if ($this->_var['icar']['csys'] == $this->_var['csys']['id']): ?>selected="selected"<?php endif; ?> value="<?php echo $this->_var['csys']['id']; ?>"><?php echo $this->_var['csys']['csys_name']; ?></option>
            <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
         </select>
        </td>
        <td bgcolor="#ffffff" align="center">
        <select name="ctype_id" id='ck'>
    	<?php $_from = $this->_var['ctype']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'ctype');if (count($_from)):
    foreach ($_from AS $this->_var['ctype']):
?>
            <option <?php if ($this->_var['icar']['ctype'] == $this->_var['ctype']['id']): ?>selected="selected"<?php endif; ?> value="<?php echo $this->_var['ctype']['id']; ?>"><?php echo $this->_var['ctype']['ctype_name']; ?></option>
        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
    	</select>
        </td>
        <td bgcolor="#ffffff" align="center">
        <?php if ($this->_var['icar']['defaultcar'] == '1'): ?>
        是
        <?php else: ?>
        <input type="checkbox" name="defaultcar" value="1" >是
        <?php endif; ?>
        ...
        <td bgcolor="#ffffff" align="center">
        <input type="hidden" name="oldtype" value="<?php echo $this->_var['icar']['ctype']; ?>" />
        <input type="hidden" name="icarid" value="<?php echo $this->_var['icar']['id']; ?>" />
          ...
          <form action="user.php" method="post" enctype="multipart/form-data" name="formMsg" onSubmit="return submitMsg()">
                  <table width="100%" border="0" cellpadding="3">
                    ...
                    <tr>
                      ...
                      <td>
                      <a href ="<?php echo $this->_var['order_info']['url']; ?>"><img src="themes/car/images/note.gif" /><?php echo $this->_var['order_info']['order_sn']; ?></a>
...
?>
    <a href="search.php?keywords=<?php echo urlencode($this->_var['tag']['tag_words']); ?>" class="f6"><?php echo htmlspecialchars($this->_var['tag']['tag_words']); ?></a> <a href="user.php?act=act_del_tag&amp;tag_words=<?php echo urlencode($this->_var['tag']['tag_words']); ?>" onclick="if (!confirm('<?php echo $this->_var['lang']['confirm_drop_tag']; ?>')) return false;" title="<?php echo $this->_var['lang']['drop']; ?>"><img src="themes/car/images/drop.gif" alt="<?php echo $this->_var['lang']['drop']; ?>" /></a>&nbsp;&nbsp;
    <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
    <?php else: ?>
...
function delicar(cid){
	var conf=window.confirm("确定删除？");
	if(conf){
		Ajax.call('user.php?act=delicar','cid='+cid, delResponse, 'POST', 'JSON');
	}
}
